...
import { AllCars } from "../../Data/mock-data/mock-cars";
...
class CarDetails extends Component<IProps & RouteComponentProps<{}>, IState> {
    ...
    if (model) {
      this.setState(state => ({
        ...
        rowData: state.rowData.filter(
          r => r.model && r.model.toLowerCase() === model.toLowerCase()
        ),
        car: state.rowData.filter(
          r => r.model && r.model.toLowerCase() === model.toLowerCase()
        )[0]
      }));
    }
  ...
  handleValueChange = (car: ICar) => {
    this.setState(state => ({
      ...
      rowData: state.items.filter(
        r => r.model && r.model.toLowerCase() === car.model.toLowerCase()
      )
    }));
    if (car) {
  ...
  filterCar: ItemPredicate<ICar> = (query, car, _index, exactMatch) => {
    ...
    if (exactMatch) {
      ...
      return (
        `${car.make.toLowerCase()}`.indexOf(normalizedQuery) !== -1 ||
  ...
  render() {
    ...
    const initialContent = this.state.hasInitialContent ? (
      <MenuItem
        disabled={true}
        text={`${this.state.rowData.length} items loaded.`}
      />
    ) : (
      undefined
    );
